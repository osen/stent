...
ref(SomeStruct) someStruct = {0};         // Using Stent
...
someStruct = create(SomeStruct);                 // Using Stent

5) To free dynamic memory:

free(someStruct);     // Standard C
destroy(someStruct);  // Using Stent
...
The only thing to remember is to add the following line to the end
of the program.

stent_cleanup();  // Scan internal memory for list of data yet to be freed
